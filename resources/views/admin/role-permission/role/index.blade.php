...
@section('content')
                        ...
                        <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-2">
                            ...
                            <div class="ltr:md:text-end rtl:md:text-start">
                                <button type="button" data-modal-target="showModal"
                                        ...
                                        class="align-bottom ri-add-line me-1"></i> Add Role
                                </button>
                            </div>
                        </div>
                            ...
                            <table class="w-full whitespace-nowrap" id="customerTable">
                                ...
                                @foreach($roles as $role=>$value)
                                    <tr>
                                        ...
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">{{++$role}}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 email">{{$value->name}}</td>

                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                            <div class="flex gap-2">
                                                <div class="edit">
                                                    <button data-id="" data-modal-target="RolePermission{{$value->id}}"
                                                        ...
                                                        Add / Edit To Permissions
                                                    </button>
                                                    <div id="RolePermission{{$value->id}}" modal-center=""
                                                         class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                                        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                                                            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                                                                <h5 class="text-16" id="exampleModalLabel">Give Permission</h5>
                                                                <button data-modal-close="RolePermission{{$value->id}}"
                                                                        class="transition-all duration-200 ease-linear text-slate-400 hover:text-slate-500"><i
                                                                        data-lucide="x" class="size-5"></i></button>
                                                            </div>
                                                            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                                                <form class="tablelist-form" action="{{route('roles.permissions', $value->id)}}" method="POST" id="edit-role">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    @foreach($permissions as $permission)
                                                                        <div class="flex items-center gap-2">
                                                                            <input id="checkboxCircle5"
                                                                                   class="border rounded-full appearance-none cursor-pointer size-4 bg-slate-100 border-slate-200 dark:bg-zink-600 dark:border-zink-500 checked:bg-yellow-500 checked:border-yellow-500 dark:checked:bg-yellow-500 dark:checked:border-yellow-500 checked:disabled:bg-yellow-400 checked:disabled:border-yellow-400"
                                                                                   type="checkbox" value="{{$permission->name}}" name="permission[]"
                                                                                   @if(in_array($permission->id, $rolePermissions[$value->id])) checked @endif>
                                                                    ...
                                                                    <div class="flex justify-end gap-2">
                                                                        ...
                                                                        <button type="button" data-modal-close="RolePermission{{$value->id}}"
                                                                                ...
                                                                                data-modal-close="RolePermission">Close
                                                                        </button>
                                                                        <button type="submit"
                                                                                class="text-white bg-green-500 border-green-500 btn hover:text-white hover:bg-green-600 hover:border-green-600 focus:text-white focus:bg-green-600 focus:border-green-600 focus:ring focus:ring-green-100 active:text-white active:bg-green-600 active:border-green-600 active:ring active:ring-green-100 dark:ring-green-400/10"
                                                                                id="add-btn">Give Permission
                                                                        </button>
                                                ...
                                                <div class="edit">
                                                    <a data-id="{{$value->id}}" data-bs-target="editRole{{$value->id}}"
                                                       data-modal-target="editRole{{$value->id}}"
                                                       href="javascript:void(0);"
                                                       class="edit-button py-1 text-xs text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20 edit-item-btn">Edit</a>
                                                    <div id="editRole{{$value->id}}" modal-center=""
                                                         class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
                                                        <div
                                                            class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                                                            <div
                                                                class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                                                                <h5 class="text-16" id="exampleModalLabel">Edit
                                                                    Role</h5>
                                                                <button type="button"
                                                                        data-modal-close="editRole{{$value->id}}"
                                                            ...
                                                            <div
                                                                ...
                                                                <form class="tablelist-form"
                                                                      action="{{route('roles.update', $value->id)}}"
                                                                      method="POST" id="edit-role">
                                                                    @csrf
                                                                    @method('PUT')

                                                                    <div class="mb-3">
                                                                        <label for="customername-field"
                                                                               class="inline-block mb-2 text-base font-medium">Role
                                                                            Name <span
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="mt-5 text-center">
                                                                    ...
                                                                    <p class="text-slate-500 dark:text-zink-200">Are you
                                                                        certain you want to delete this role?</p>
    ...
    <div id="showModal" modal-center=""
        ...
        <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form class="tablelist-form" action="{{route('roles.store')}}" method="POST">
                    @csrf

                    <div class="mb-3">
                        <label for="customername-field" class="inline-block mb-2 text-base font-medium">Role Name <span
                                class="text-red-500">*</span></label>
